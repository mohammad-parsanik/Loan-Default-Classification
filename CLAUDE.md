...
- `MODEL_ARMS` / `DEPLOY_ARM` / `DEEPSETS_ENABLED` — the arms lineup ("multiclass", "binary", "ordinal", "per_cat"; builders in `aggregated_xgboost.ARM_BUILDERS`), how the deployed arm is picked ("auto" = best pooled ranking AP among full-distribution arms; a final fit needs an explicit name), and the legacy neural arm toggle. The "binary" arm is a ranking-ceiling diagnostic only — no class distribution, never deployed (business requires per-class probabilities; queue still sorts by P3).
...
- **KNOWN GAP (Phase C pending):** `Predictor`/`ModelLoader` still score via the LEGACY DeepSets artifacts (`deep_sets.pt` + embeddings + `model_bundle.pkl`) — they have NOT yet been reworked for the new `model_arm.pkl` deployment artifacts. `python run.py predict` only works against a DeepSets-era artifact dir/bundle until the rework lands (planned after the Run-6 arms shootout picks the winner).
