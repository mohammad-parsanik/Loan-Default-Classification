...
- **Standalone deployment:** `build_scoring_package.py --bundle <model_bundle.pkl> --output <dir>` copies just the ~16 files `score_dataframe()` needs (no `run.py`, no `src/model/*` DeepSets code, no torch/optuna/umap) into a self-contained folder with its own `requirements-scoring.txt`, for handing scoring to a system that shouldn't depend on this whole repo. Regression-tested by actually running scored output in a subprocess with this repo off `sys.path` and torch/optuna/pyodbc import-blocked (`test_build_scoring_package_runs_standalone`). See `DEPLOYMENT.md` §4.
