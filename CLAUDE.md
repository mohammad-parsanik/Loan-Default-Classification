...
- **Inference architecture:** `ModelLoader.load_pipeline()` returns `(scorer, calibrator, features)`. A `Scorer` abstracts the model family — `ArmScorer` (scale → `aggregate_features` → `arm.predict_proba`; the deployed path, no torch) or `DeepSetsScorer` (legacy embeddings → meta-learner). `Predictor` calls `self.scorer.raw_probs(instances)` then calibrate → mask → queue. The directory loader prefers `model_arm.pkl`, falling back to DeepSets artifacts.
