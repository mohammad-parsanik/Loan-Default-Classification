...
- **Oracle Native Integration:** Built-in connection layer using `cx_Oracle` via direct `host:port/service_name` strings.
...
> **Note:** The `cx_Oracle` package requires Oracle Instant Client (e.g., `OraClient12Home1`) to be installed and present in your system's `PATH` / `LD_LIBRARY_PATH`.

## Project Structure
- `project_config.py`: Core hyperparameters, table names, and paths.
- `src/db/`: Oracle `cx_Oracle` connector.
