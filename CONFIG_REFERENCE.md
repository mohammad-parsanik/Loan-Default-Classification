...
| `TRAIN_WINDOW_SNAPSHOTS` | `None` | Narrows the eligible training window. `None` keeps every snapshot clearing the `LABEL_HORIZON_MONTHS` gap; a positive `N` keeps the **newest** `N`, a negative `N` the **oldest** `N`. Exists for one experiment (`AGENT_HANDOFF.md` §24): the upstream installment table is hard-deleted for closed and post-NPL loans, so old snapshots were rebuilt after more of their future-severe rows had vanished. Training on the newest vs oldest half and scoring both on the same test fold says whether that depletion costs **ranking** or only **calibration** — uniform undersampling of positives shifts the intercept and leaves the ordering alone, feature-dependent deletion does not. Applied to both the single-split and walk-forward paths; in walk-forward it narrows *before* the `MIN_TRAIN_SNAPSHOTS` check. **Leave at `None` for production — it discards training data by design.** |
...
| `EXPOSURE_FEATURE` | `"REMAINING_AMNT"` | The deployed arm's ranking block is additionally cut by decile of this feature, reported as `ranking_by_exposure` and logged with the AP spread across deciles. This is the **amount-banding test**: a separate model per amount band is worth building only when the feature→target *relationship* differs, not when the base rate does (a base-rate shift is one tree split, at a threshold the data picks). Flat lift@K across deciles ⇒ banding buys nothing. It also watches the §24 deletion bias — if post-NPL deletion varies with loan size, the model learns a size-dependent distortion as if it were signal. Read `pr_auc` and `lift`, **not** `recall`: K is the whole API budget spent inside a single decile. Deciles are cut on the ranked population only, rank-based so a skewed amount distribution still yields balanced bins. `None` skips the slice. |
